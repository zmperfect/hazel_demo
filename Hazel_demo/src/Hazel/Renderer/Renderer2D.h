...
namespace Hazel {
    ...
    public:
        ...
        static void DrawCircle(const glm::mat4& transform, const glm::vec4& color, float thickness = 1.0f, float fade = 0.005f, int entityID = -1);//绘制圆形

        static void DrawLine(const glm::vec3& p0, glm::vec3& p1, const glm::vec4& color, int entityID = -1);// 绘制线

        static void DrawRect(const glm::vec3& position, const glm::vec2& size, const glm::vec4& color, int entityID = -1);// 绘制矩形
        static void DrawRect(const glm::mat4& transform, const glm::vec4& color, int entityID = -1);// 绘制矩形

        static void DrawSprite(const glm::mat4& transform, SpriteRendererComponent& src, int entityID = -1);//绘制精灵

        static float GetLineWidth();// 获取线宽

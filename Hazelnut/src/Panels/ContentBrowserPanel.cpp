...
namespace Hazel {
    ...
    void ContentBrowserPanel::OnImGuiRender()
    {
        ...
        for (auto& directoryEntry : std::filesystem::directory_iterator(m_CurrentDirectory))
        {
            ...
            ImGui::PushID(filenameString.c_str());// 设置按钮的 ID
            Ref<Texture2D> icon = directoryEntry.is_directory() ? m_DirectoryIcon : m_FileIcon;//设置图标
            ImGui::PushStyleColor(ImGuiCol_Button, ImVec4{ 0, 0, 0, 0 });// 设置按钮的颜色
            ...
            if (ImGui::BeginDragDropSource())
			{
				...
				ImGui::SetDragDropPayload("CONTENT_BROWSER_ITEM", itemPath, (wcslen(itemPath) + 1) * sizeof(wchar_t));// 设置拖放的数据
